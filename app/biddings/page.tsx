...
function AuctionBidPageContent() {
  ...
  // Property Metadata
  const [propertyMeta, setPropertyMeta] = useState<AuctionMeta | null>(null);
    ...
    try {
      const results = await publicClient.multicall({
        contracts: [
          ...
          { address: auctionAddress, abi: auctionAbi, functionName: "minBidIncrementPercent" },
      ...
      if (walletAddress) {
        const userResults = await publicClient.multicall({
          contracts: [
            ...
            { address: auctionAddress, abi: auctionAbi, functionName: "hasPaid", args: [walletAddress] },
  ...
  // Fetch property metadata from NFT tokenURI
  useEffect(() => {
    if (!auctionAddress) return;
    let cancelled = false;
    (async () => {
      ...
      try {
        const auctionResults = await publicClient.multicall({
          contracts: [
            { address: auctionAddress, abi: auctionAbi, functionName: "nftContract" },
            { address: auctionAddress, abi: auctionAbi, functionName: "tokenId" },
          ],
        });
        const nftContract = auctionResults[0].result as `0x${string}` | undefined;
        const tokenId = auctionResults[1].result as bigint | undefined;
        if (!nftContract || tokenId == null || cancelled) return;
        ...
        if (!tokenURIResult || cancelled) return;

        const meta = await fetchNFTMetadataFull(tokenURIResult as string);
        if (!cancelled) setPropertyMeta(meta);
      } catch (err) {
        console.warn("Failed to fetch property metadata:", err);
      } finally {
        if (!cancelled) setMetaLoading(false);
      }
    })();
    return () => { cancelled = true; };
  }, [auctionAddress, publicClient]);
